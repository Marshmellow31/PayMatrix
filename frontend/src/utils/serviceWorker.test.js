...
function worker() {
  ...
  const context = {
    URL, console, clients,
    self: { location: { origin: 'https://pay-matrix.vercel.app' }, __WB_MANIFEST: [], addEventListener: (name, fn) => { events[name] = fn; } },
    precacheAndRoute: vi.fn(), cleanupOutdatedCaches: vi.fn(), clientsClaim: vi.fn(),
    ...
    CacheFirst: class {}, StaleWhileRevalidate: class {}, ExpirationPlugin: class {}, CacheableResponsePlugin: class {},
  };
...
describe('installed service worker', () => {
  it('serves a deep navigation from the installed shell without a network strategy', async () => {
    ...
    expect(await navigation.handler({ request: { url: '/groups/trip' } })).toBe('cached release HTML');
    expect(shell).toHaveBeenCalledOnce();
    expect(navigation.options.denylist.some((pattern) => pattern.test('/api/scan-bill'))).toBe(true);
  });
  it('awaits navigation before focusing an existing notification window', async () => {
    const { events, clients } = worker();
    const order = [];
    clients.matchAll.mockResolvedValue([{ url: 'https://pay-matrix.vercel.app/dashboard',
      navigate: (url) => { order.push(url); return Promise.resolve({ focus: () => Promise.resolve(order.push('focus')) }); }, focus: vi.fn(),
    }]);
    let done;
    events.notificationclick({ notification: { close() {}, data: { type: 'friend_request' } }, waitUntil: (promise) => { done = promise; } });
    await done;
  ...
  it('opens a safe fallback for external notification URLs', async () => {
    const { events, clients } = worker();
    let done;
    events.notificationclick({ notification: { close() {}, data: { url: 'https://evil.test' } }, waitUntil: (promise) => { done = promise; } });
    await done;
