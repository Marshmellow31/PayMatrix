...
const groupService = {
  ...
  getGroups: async (userId) => {
    ...
    const allGroups = await Promise.all(
      querySnapshot.docs.map((doc) => groupService.expandGroupData(doc))
    );
  ...
  deleteGroup: async (id, _userId) => {
    try {
      ...
      if (hasPending) {
        throw new Error(
          'Cannot delete group with pending settle-ups. Please reconcile all accounts first.'
        );
      }
      ...
      return wrap({
        message: 'Group archived successfully. Members can still export data from Settings.',
      });
    } catch (err) {
  ...
  addMember: async (groupId, data) => {
    ...
    if (!userIdToAdd && data.email) {
      ...
      if (snap.empty)
        throw new Error('User not found in PayMatrix system. They must sign up first.');
      userIdToAdd = snap.docs[0].id;
    }

    const finalUid =
      userIdToAdd && typeof userIdToAdd === 'object'
        ? userIdToAdd._id || userIdToAdd.uid
        : userIdToAdd;

    if (!finalUid || typeof finalUid !== 'string' || finalUid === 'undefined')
