...
const MemberList = ({
  ...
  useEffect(() => {
    ...
    if (selectedMember && !selectedMember.isMe) {
      ...
      friendService
        .checkRelationship(selectedMember._id || selectedMember.uid || selectedMember)
        .then((res) => {
          if (isMounted) {
    ...
    <>
      <div className="flex flex-col gap-2">
        {uniqueMembers.map((member) => {
          ...
          const userName =
            user.name || (userIdStr === currentUserId?.toString() ? 'You' : 'Member');
            ...
            >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold text-on-surface font-inter group-hover:text-primary transition-colors truncate">
                    ...
                    {userIdStr === currentUserId?.toString() && (
                      <span className="ml-2 text-[9px] bg-primary/20 text-primary px-1.5 py-0.5 rounded uppercase font-bold shrink-0">
                        You
                      </span>
                    )}
                  </p>
              ...
              <div className="flex items-center gap-3 shrink-0 ml-3">
                ...
                {ChevronRight && (
                  <ChevronRight
                    size={14}
                    className="text-on-surface-variant opacity-20 group-hover:opacity-100 transition-opacity"
                  />
                )}
              </div>
            </button>
          ...
          {selectedMember && (
            <div className="flex flex-col">
              <div className="text-center mb-10 pt-2">
                ...
                <h3 className="text-2xl font-bold font-manrope text-[#e5e2e1] tracking-tight italic-none">
                  {selectedMember.name}
                </h3>
                <p className="text-[10px] text-[#919191] font-bold uppercase tracking-[0.2em] mt-1.5 font-inter">
                  {selectedMember._id?.toString() === adminId?.toString()
                    ? 'Group Admin'
                    : 'Member'}
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#242424] border border-white/[0.03] transition-colors hover:border-white/5 group">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="text-[9px] uppercase tracking-widest font-black text-[#666666] mb-0.5 font-manrope">
                      Entity Identifier
                    </span>
                    <span className="text-sm text-[#e3e2e2] truncate font-medium font-inter">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm text-[#e3e2e2] font-medium font-inter">
                      {selectedMember.updatedAt
                        ? formatDistanceToNow(new Date(selectedMember.updatedAt), {
                            addSuffix: true,
                          })
                        : 'Live Action'}
                    </span>
