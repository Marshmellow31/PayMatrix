  ...
  return (
    <div className="flex flex-col gap-8 sm:gap-10 lg:grid lg:grid-cols-[0.8fr_1.2fr] lg:items-center lg:gap-7 my-auto w-full max-w-6xl">
      ...
      <motion.div layout className="relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#171717]/90 p-3 shadow-[0_30px_100px_rgba(0,0,0,0.4)] backdrop-blur-xl sm:rounded-[2rem] sm:p-6">
        ...
        <div className="relative flex items-center justify-between border-b border-white/[0.08] pb-3 sm:pb-5"><div><p className="text-base font-black tracking-tight sm:text-lg">{TRIAL_GROUP.name}</p><p className="mt-1 text-[10px] text-white/35 sm:text-xs">{TRIAL_GROUP.subtitle}</p></div><div className="flex -space-x-2">{TRIAL_MEMBERS.map((member) => <div key={member.id} aria-label={`${member.name} avatar`} className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#171717] text-sm sm:h-8 sm:w-8 ${member.color}`}>{member.avatar}</div>)}</div></div>
        <div className="relative mt-3 rounded-2xl border border-white/[0.08] bg-white/[0.035] p-3 sm:mt-5 sm:p-4"><div className="flex items-start justify-between"><div className="flex items-center gap-2 sm:gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-300/10 text-amber-200 sm:h-10 sm:w-10"><ReceiptText size={16} /></div><div><p className="text-xs font-bold sm:text-sm">Beach shack dinner</p><p className="mt-1 text-[10px] text-white/35 sm:text-[11px]">Paid by Maya · split unevenly</p></div></div><span className="text-xs font-black sm:text-sm">₹2,460</span></div><button onClick={onAddExpense} disabled={expenseAdded} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] py-2.5 text-[10px] font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/[0.12] disabled:cursor-default disabled:text-emerald-200 sm:mt-4 sm:py-3 sm:text-xs">{expenseAdded ? <><Check size={15} /> Balances updated</> : <>Add this sample expense <ArrowRight size={14} /></>}</button></div>
