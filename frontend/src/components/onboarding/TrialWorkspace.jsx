...
const TrialWorkspace = ({ expenseAdded, onAddExpense, onContinue }) => {
  ...
  return (
    <div className="grid w-full max-w-6xl gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
      <div className="max-w-md">
        ...
        <h1 className="font-manrope text-4xl font-black leading-[0.98] tracking-[-0.05em] sm:text-6xl">Make one expense.<br /><span className="text-white/35">See the clarity.</span></h1>
        <p className="mt-6 max-w-sm text-sm leading-7 text-white/50">This is what PayMatrix does for your group. Tap the sample expense and watch the balances resolve.</p>
        <button onClick={onContinue} className="mt-8 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-white/55 transition hover:text-white">Explore the features <ArrowRight size={15} /></button>
      </div>

      <motion.div layout className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#171717]/90 p-4 shadow-[0_30px_100px_rgba(0,0,0,0.4)] backdrop-blur-xl sm:p-6">
        ...
        <div className="relative mt-5 rounded-2xl border border-white/[0.08] bg-white/[0.035] p-4"><div className="flex items-start justify-between"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-300/10 text-amber-200"><ReceiptText size={18} /></div><div><p className="text-sm font-bold">Beach shack dinner</p><p className="mt-1 text-[11px] text-white/35">Paid by Maya · split equally</p></div></div><span className="text-sm font-black">₹2,460</span></div><button onClick={onAddExpense} disabled={expenseAdded} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] py-3 text-xs font-black uppercase tracking-[0.12em] text-white transition hover:bg-white/[0.12] disabled:cursor-default disabled:text-emerald-200">{expenseAdded ? <><Check size={15} /> Balances updated</> : <>Tap to record sample expense <ArrowRight size={14} /></>}</button></div>
