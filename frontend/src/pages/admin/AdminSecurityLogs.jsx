...
const LogRow = ({ log }) => {
  ...
  return (
    ...
    >
      <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 bg-white/5 border border-white/10" style={{ background: `${color}15` }}>
        {eventIcon(log.event)}
      </div>
...
const AdminSecurityLogs = () => {
  ...
  const loadLogs = useCallback(async (reset = false) => {
    try {
      ...
      const res = await adminService.getSecurityLogs(30, reset ? null : lastDoc, eventFilter || null);
  ...
  useEffect(() => { loadLogs(true); }, [eventFilter]); // eslint-disable-line

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-black font-manrope text-white tracking-tight">Security Logs</h1>
          <p className="text-sm text-white/40 mt-0.5">Immutable audit trail of security events</p>
        </div>
        <button onClick={() => loadLogs(true)} className="p-2 rounded-xl hover:bg-white/5 text-white/40 hover:text-white/70 transition-colors" title="Refresh">
          <RefreshCw size={15} />
        </button>
      </div>
      ...
      <div className="flex flex-col gap-2.5 mb-6">
        <div className="flex items-center gap-2 text-white/30">
          <Filter size={13} />
          <span className="text-[10px] font-bold uppercase tracking-widest font-inter">Filter events</span>
        </div>
      ...
      {loading ? (
        ...
        >
          <div className="flex items-center gap-2 px-4 py-3 bg-white/[0.01] border-b border-white/[0.04]">
            ...
            <span className="text-[10px] font-bold text-white/45 uppercase tracking-widest">{logs.length} events logged</span>
          </div>
