...
const StatusBadge = ({ status }) => {
  const map = {
    active:   { color: '#22c55e', label: 'Active', bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.2)' },
    archived: { color: '#eab308', label: 'Archived', bg: 'rgba(234,179,8,0.1)', border: 'rgba(234,179,8,0.2)' },
    deleted:  { color: '#ef4444', label: 'Deleted', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.2)' },
  };
  const { color, label, bg, border } = map[status] || { color: '#e5e2e1', label: status || 'Active', bg: 'rgba(255,255,255,0.05)', border: 'rgba(255,255,255,0.1)' };
  return (
...
const GroupDetailDrawer = ({ groupId, onClose }) => {
  ...
  useEffect(() => {
    adminService.getGroupDetails(groupId)
      .then(setData)
      .catch(() => setData(null))
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div className="flex-1 flex items-center justify-center text-white/30 text-sm">Group not found</div>
      ) : (
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {[
            ...
            { label: 'Created',     value: data.group.createdAt ? new Date(data.group.createdAt).toLocaleDateString('en-IN') : '—' },
          ].map((row) => (
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {data.expenses.slice(0, 8).map((e) => (
                <div key={e._id} className="flex items-center justify-between rounded-xl px-3 py-2 border border-white/[0.02] bg-white/[0.04]">
                  <span className="text-xs text-white/70 truncate flex-1">{e.title}</span>
                  <span className="text-xs font-bold ml-3 text-orange-400">₹{e.amount?.toLocaleString('en-IN') || '—'}</span>
                </div>
              ))}
              {data.expenses.length === 0 && <p className="text-xs text-white/25 italic">No expenses</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {data.settlements.slice(0, 5).map((s) => (
                <div key={s._id} className="flex items-center justify-between rounded-xl px-3 py-2 border border-white/[0.02] bg-white/[0.04]">
                  <span className="text-xs text-white/60 truncate">{s.payerName} → {s.receiverName}</span>
                  <span className="text-xs font-bold ml-2 text-emerald-400">₹{s.amount?.toLocaleString('en-IN')}</span>
                </div>
              ))}
              {data.settlements.length === 0 && <p className="text-xs text-white/25 italic">No settlements</p>}
            </div>
          </div>
        </div>
      )}
...
const AdminGroups = () => {
  ...
  useEffect(() => { loadGroups(true); }, []); // eslint-disable-line
          ...
          <div className="divide-y divide-white/[0.04]">
            {filtered.map((group) => (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="grid grid-cols-[1fr_auto_auto_auto] gap-4 items-center px-4 py-3 hover:bg-white/[0.02] transition-colors"
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium text-white truncate">{group.name || group.title || 'Unnamed'}</p>
                  <p className="text-xs text-white/30 truncate">{group.category || 'No category'} · {group.createdAt ? new Date(group.createdAt).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' }) : '—'}</p>
                </div>
