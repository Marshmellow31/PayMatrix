...
object Money {
    ...
    fun format(paise: Long): String = format(paise, "INR")

    fun format(paise: Long, currency: String): String {
        val upperCurrency = currency.trim().uppercase(Locale.US).ifEmpty { "INR" }
        val locale = when (upperCurrency) {
            "INR" -> Locale("en", "IN")
            "USD" -> Locale.US
            "EUR" -> Locale.GERMANY
            "GBP" -> Locale.UK
        ...
        try {
            format.currency = java.util.Currency.getInstance(upperCurrency)
        } catch (_: Exception) {}
