...
class PayMatrixMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        val notificationId = (message.data["notificationId"] ?: message.messageId)?.hashCode() ?: System.currentTimeMillis().toInt()
        val intent = Intent(this, MainActivity::class.java)
            .addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP)
            .putExtra("quick_destination", NotificationDestination.resolve(message.data["type"].orEmpty(), message.data["groupId"].orEmpty(), message.data["url"].orEmpty()))
        val pending = PendingIntent.getActivity(this, notificationId, intent, PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT)
        manager.notify(
            notificationId,
