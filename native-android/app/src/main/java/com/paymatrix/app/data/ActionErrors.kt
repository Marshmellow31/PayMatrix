...
internal fun isNoGoogleCredentialAvailable(error: Throwable): Boolean =
    error is GetCredentialException && (
        error.message.orEmpty().contains("no credential", ignoreCase = true) ||
            error.type.contains("NoCredential", ignoreCase = true) ||
            error.type.contains("NO_CREDENTIAL", ignoreCase = true)
        )

internal fun isRecoverableGoogleCredentialFailure(error: Throwable): Boolean =
    isGoogleReauthFailure(error) || isNoGoogleCredentialAvailable(error)
...
internal fun actionErrorMessage(error: Throwable): String = when (error) {
    ...
    is GetCredentialException -> {
        ...
        when {
            ...
            msg.contains("16") || isNoGoogleCredentialAvailable(error) ->
                "Google could not open an eligible account. Check Google Play services, then retry or use email sign-in."
            else ->
