...
class AuthRepository(
    ...
    suspend fun currentVerifiedProfile(): UserProfile? {
        ...
        return profile(user.uid, cachedFirst = true)
    }
    ...
    private suspend fun completeSignIn(firebaseUser: FirebaseUser, isGoogle: Boolean = false): UserProfile {
        ...
        val resolvedName = (firebaseUser.displayName?.ifBlank { null }
            ?: existing.getString("name")?.ifBlank { null }
            ?: "Member").take(50)
    ...
    suspend fun profile(uid: String = currentUser?.uid ?: error("Authentication required"), cachedFirst: Boolean = false): UserProfile {
        val ref = db.collection("users").document(uid)
        val cached = if (cachedFirst) runCatching { ref.get(com.google.firebase.firestore.Source.CACHE).await() }.getOrNull() else null
        val doc = cached?.takeIf { it.exists() } ?: ref.get().await()
    ...
    private suspend fun googleCredential(context: Context, webClientId: String): com.google.firebase.auth.AuthCredential {
        ...
        // This method is called from an explicit "Continue with Google" button.
        // Google's button flow can surface accounts that the passive bottom-sheet
        // discovery flow excludes (for example, accounts needing reauthentication).
        val custom = recoverGoogleCredential(
            request = { googleButtonCredential(credentialManager, context, webClientId) },
            clearState = { credentialManager.clearCredentialState(ClearCredentialStateRequest()) },
            // A stale Play Services session is retried once with the broad account
            // chooser. User cancellation and coroutine cancellation are never retried.
            retry = { googleAccountChooserCredential(credentialManager, context, webClientId) },
        )
    ...
    private suspend fun googleButtonCredential(
        credentialManager: CredentialManager,
        context: Context,
        webClientId: String,
    ): CustomCredential {
        val option = GetSignInWithGoogleOption.Builder(webClientId).build()
        return requestGoogleCredential(credentialManager, context, option)
    }

    private suspend fun googleAccountChooserCredential(
        credentialManager: CredentialManager,
        context: Context,
        webClientId: String,
    ): CustomCredential {
