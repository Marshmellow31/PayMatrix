...
class ActionRecoveryTest {
    ...
    @Test fun noCredentialStateClearsAndFallsBackOnce() = runTest {
        val events = mutableListOf<String>()
        val result = recoverGoogleCredential(
            request = { events.add("button"); throw NoCredentialException("No credentials available") },
            clearState = { events.add("clear") },
            retry = { events.add("chooser"); "credential" },
        )
        assertEquals("credential", result)
        assertEquals(listOf("button", "clear", "chooser"), events)
    }
    ...
    @Test fun noCredentialFailureDoesNotIncorrectlyClaimTheDeviceHasNoGoogleAccount() {
        val message = actionErrorMessage(NoCredentialException("No credentials available"))
        assertTrue(message.contains("Google Play services"))
        assertTrue(message.contains("retry"))
        assertFalse(message.contains("No Google accounts found"))
    }
