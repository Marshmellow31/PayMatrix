...
**Status:** Accepted for production migration in v2.0.2
...
The native client uses the production package `com.paymatrix.app`, version code `20200`, and the v1.2.5 release key. This makes v2.0.2 an in-place Android upgrade instead of a second side-by-side application.
...
- Installing v2.0.2 replaces an installed v1.2.5 or v2.0.1 because Android sees the same signed application with a higher version code.
- A rollback to an older APK requires uninstalling v2.0.2, which can clear local-only app data; cloud data remains in Firebase.
